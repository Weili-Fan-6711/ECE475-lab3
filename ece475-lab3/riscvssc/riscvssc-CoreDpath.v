...
(
  ...
  wire [31:0] next_pc_expected_Phl;
  ...
  // PC flows based on the Steering Logic AND validity
  // To compute branch targets accurately, we need the PC of the instruction
  // actually executing in Pipeline A.
  // 
  // If steer_inst0_to_B_Dhl is TRUE:
  //   - A gets ir1 (so PC+4), B gets ir0 (so PC)
  // If steer_inst0_to_B_Dhl is FALSE:
  //   - If ir0 is valid, A gets ir0 (PC), B gets ir1 (PC+4)
  //   - If ir0 is invalid but ir1 is valid, A gets ir1 (PC+4)!
  ...
  // opA0_byp_mux_out_Dhl and instA_imm_i_Dhl are already from the issued instruction.
